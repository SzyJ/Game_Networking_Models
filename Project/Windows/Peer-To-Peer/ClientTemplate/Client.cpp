...
int main() {
	...
	int bytesSent = sendto(connectSocket, request.c_str(), request.length(), 0, (sockaddr*)&serverAddr, serverSize);
	std::cout << "Bytes sent to: " << bytesSent << std::endl;
	...
	inet_pton(AF_INET, host.c_str(), &otherAddr.sin_addr);
